...
| Browser UI (`index.html`) | Done. Tested headless: search, filters, moments, pagination, shareable URLs, both themes. |
...
- The static site searches transcripts through the inverted index but does not
  rank whole talks by transcript relevance as well as the CLI does; the CLI is
  the better tool for precision work.
